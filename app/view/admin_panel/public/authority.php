  ...
  <div class="outer">
    <div class="inner bg-light lter">
      <div class="text-center">
        <a href="/add_authority?admin_id=<?=$admin->id?>">
          <button type="button" class="btn btn-success pull-right" style="z-index:99;">  
            <i class="fa fa-plus-circle btn_add" aria-hidden="true"></i> Create Authority </button></a>
            <ul class="stats_box">
              <?php if($authorities!='null') {?>
                <?php foreach ($authorities as $key => $value) { ?>
                  <a href="<?php echo '/examination?authority_id='.$value->id ?>">  
                    <li class="list_change">
                      <div class="sparkline bar_week"></div>
                      <div class="stat_text">
                        <strong><?php echo $value->name?></strong>
                        <?php //foreach ($examinations as $key1 => $value1) { ?>
                          <!--<span class="percent down"> <i class="fa fa-caret-down"></i>--> <?php //echo $value1->total ?></span>
                          <?php //}?>
                        </div>
                        <a href="/edit_authority?admin_id=<?=$admin->id?>&authority_id=<?php echo $value->id?>">
