...
public static function outputCSV($data,$file_name = 'file.csv') {
    ...
    public static function exportToExl_2($id){
			...
			/*$setSql = "SELECT a.user_id,c.name,c.roll_num,c.roll_code,c.reference_id,sum(if(a.ansoption=b.ansoption,1,0)) as score FROM `user_answers` a left join questions b on a.ques_id=b.id 
			left join users c on c.id=a.user_id
			where b.quiz_id=1 group by a.user_id";*/

			// $setSql = "SELECT ua.user_id,u.reg_num,u.name,u.gender,u.dob,u.category,u.roll_num,u.roll_code,u.reference_id,IF(u.is_login=1, 'P', 'A') AS Present,sum(if(ua.ansoption=q.ansoption,1,0)) as Correct,sum(if(ua.ansoption!=q.ansoption,1,0)) as InCorrect,sum(if(ua.ansoption='',1,0)) as Blank,sum(if(ua.ansoption=q.ansoption,1,0)) as Score,GROUP_CONCAT(if(ua.ansoption='','x',ua.ansoption))  AS Answer_options  FROM `user_answers` ua left join questions q on ua.ques_id=q.id 
   ...
   where q.quiz_id=1 group by ua.user_id";
     ...
     public static function post_result_list_csv(){
			...
			where q.quiz_id=1 group by ua.user_id";
			// echo $setSql;die;
			 // $setSql ="SELECT u.*,ua.* FROM user_answers ua left join users u on u.id=ua.user_id";
    //          echo $setSql;die;
			$setRec = mysql_query($setSql);
			$setCounter = mysql_num_fields($setRec);
			for ($i = 0; $i < $setCounter; $i++) {
