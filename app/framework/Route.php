...
{	
	...
	private function addroute($uri,$param,$verbs,$before){
		...
		$this->pattern[$pattern['pattern']][$verbs]['filter']=$before;
	}
	...
	public function get($uri, $params,$before=null){
		$this->addroute($uri, $params,'GET',$before);
	}

	public function post($uri, $params,$before=null){
		$this->addroute($uri, $params,'POST',$before);
	}
	...
	public function run(){
		...
		if ($regex=$this->match($request_uri,$verbs)) {
			...
			$before=$this->pattern[$regex][$verbs]['filter'];
			preg_match($regex, $request_uri, $this->args);
			unset($this->args[0]);
			//self::callBefore($before,$this->args);
			...
			if(Helper::isAjax()){
				return Json::make('1','URI not Found')->withError(302)->response();	
			}else{
	...
	private static function callBefore($before,$urlParam){
		@list($method,$args)=explode('@', $before);
		if (strlen($args)) {
			@$params=explode('|', $args);
		}else{
			$params=array();
		}
		$params=array_merge($params,$urlParam);
			return call_user_func_array(array(new Before,$method), $params);	
		die;
	}
}